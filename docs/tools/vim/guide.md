...
| `dd` | 删除当前行 |
| `dgg` | 删除当前行和之前的所有行 |
| `cgg` | 删除当前行和之前的所有行, 并进入插入模式 |
| `dG` | 删除当前行和之后的所有行 |
| `cG` | 删除当前行和之后的所有行, 并进入插入模式 |
| `u` | 撤消上一修改 |
| `U` | 撤消当前行的所有修改 |
| `Ctrl+R` | 重新执行上一撤消的修改 |
| `P` | 在光标前粘贴 |
...
| 快捷键 | 说明 |
| --- | --- |
| `Ctrl+D` | 命令行模式下显示补全命令列表；插入模式下减少缩进(增加为`Ctrl+T`)；普通模式下向下滚半屏 |
...
| `:help` | 进入帮助文档 |
| `:help w` | 查看w命令的帮助 |
| `:help c_CTRL-D` | 查看CTRL-D的帮助 |
| `:help insert-index` | 查看插入模式索引 |
